...
import futbol.PantallaAnadirEquipo;
import futbol.PantallaEliminarEquipos;
import futbol.PantallaGestionTorneo;
import futbol.PantallaGestionarPartido;
...
public class PantallaGestionTorneoBaloncesto extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.baloncesto_activity_pantalla_gestion_torneo);
		
		id_torneo_intent = getIntent().getIntExtra("id_torneo",-1);
		
		//Se asignan las referencias de las vistas en el layout
		llJornada = (LinearLayout) findViewById(R.id.llJornadaBC);
		...
		spnJornada = (Spinner) findViewById(R.id.spnGestionTorneoJornadaBC);
		...
		btnActualizarClasificacion.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if(PantallaInicial.bd.actualizarPartidosClasificacion(id_torneo_intent, jornada_actual, 3)){
		...
		txtTitulo = (TextView) findViewById(R.id.txtGestionTorneoBCNombre);
		txtTitulo.setText(getIntent().getStringExtra("nombre"));
		
		btnAnadirEquipo = (Button) findViewById(R.id.btnGestionTorneoBCAnadirEquipo);
		btnAnadirEquipo.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(getApplicationContext(), PantallaAnadirEquipoBaloncesto.class);
				intent.putExtra("id_torneo", id_torneo_intent);
				startActivity(intent);
			}
		});
		
		btnEliminarEquipo = (Button) findViewById(R.id.btnGestionTorneoBCEliminarEquipo);
		btnEliminarEquipo.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(getApplicationContext(), PantallaEliminarEquiposBaloncesto.class);
				intent.putExtra("id_torneo", id_torneo_intent);
				startActivity(intent);
			}
		});
		
		btnGenerarCalendario = (Button) findViewById(R.id.btnGenerarCalendarioBC);
		btnGenerarCalendario.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(PantallaInicial.bd.comprobarCalendarioTorneo(id_torneo_intent)==true){ //Ya existe el calendario, entonces pregunta si lo quiere reemplazar
					...
					AlertDialog.Builder builder = new AlertDialog.Builder(PantallaGestionTorneoBaloncesto.this);
	...
	public void generarCalendario(){
		lista_equipos = PantallaInicial.bd.obtenerEquiposDeTorneo(id_torneo_intent, 3);
		desordenarListaEquipos();
		int num_equipos = lista_equipos.size();
		if(num_equipos>1){
			if(num_equipos%2!=0){
				lista_equipos.add(new Equipo(-1, "Descansa", 0, 0, "", 3, "", 0));
				num_equipos++;
			}
			...
			for(int i=0; i<(num_equipos-1); i++){
				...
				PantallaInicial.bd.insertarPartidosTorneo(id_torneo_intent, lista_ida, 3/*Baloncesto*/, "Jornada " + (i+1), 1/*Ida*/, local/*Para saber si el equipo juega de local o no*/);
			...
			if(soloIda == false){ //Repito el proceso si ha elegido Ida y Vuelta.
				...
				for(int i=0; i<(num_equipos-1); i++){
					...
					PantallaInicial.bd.insertarPartidosTorneo(id_torneo_intent, lista_vuelta, 3/*Baloncesto*/, "Jornada " + (i+num_equipos), 2/*Vuelta*/, local);
	...
	public void cargarCalendario(int num_jornada){
		...
		for(int i=0; i<num_equipos;i=i+2){
			...
			Equipo visit = new Equipo(-1, "Descansa", 0, 0, "", 3, "", 0);
			if(i+1 < num_equipos){
				visit = PantallaInicial.bd.obtenerEquipo(lista_jornada.get(i+1));
			}
			LayoutInflater inflater = (LayoutInflater) getLayoutInflater();
			LinearLayout ll = (LinearLayout) inflater.inflate(R.layout.baloncesto_fila_jornada, null);
			TextView txtLocal = (TextView) ll.findViewById(R.id.txtJornadaEquipoLocalBC);
			txtLocal.setText(local.getNom());
			TextView txtLocalRes = (TextView) ll.findViewById(R.id.txtJornadaEquipoLocalResBC);
			txtLocalRes.setText(PantallaInicial.bd.obtenerResultado(id_torneo_intent, local.getId(), num_jornada)+"");
			TextView txtVisit = (TextView) ll.findViewById(R.id.txtJornadaEquipoVisitBC);
			txtVisit.setText(visit.getNom());
			TextView txtVisitRes = (TextView) ll.findViewById(R.id.txtJornadaEquipoVisitResBC);
			...
			ImageButton btnEditar = (ImageButton) ll.findViewById(R.id.btnJornadaEditarBC);
			...
			btnEditar.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					Intent intent = new Intent(getApplicationContext(), PantallaGestionarPartidoBaloncesto.class);
					intent.putExtra("id_torneo", id_torneo_intent);
	...
	public void calcularNumJornadas(){
		...
		numJornadas = num_filas/num_equipos; //Hago esta división porque en la BD por cada partido se crean dos registros (uno por equipo)
	}
	...
	public void DialogoModoCalendario(){
		...
		radioGroup = new RadioGroup(PantallaGestionTorneoBaloncesto.this);
		radioGroup.setBackgroundColor(Color.parseColor(getString(R.color.negro)));
		RadioButton rb1 = new RadioButton(PantallaGestionTorneoBaloncesto.this);
		rb1.setTextColor(Color.parseColor(getString(R.color.blanco)));
		rb1.setText(getString(R.string.ida));
		rb1.setId(0);
		radioGroup.addView(rb1);
		rb1 = new RadioButton(PantallaGestionTorneoBaloncesto.this);
		rb1.setTextColor(Color.parseColor(getString(R.color.blanco)));
