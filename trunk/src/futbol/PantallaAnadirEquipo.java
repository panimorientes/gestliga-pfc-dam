//Esta actividad es para añadir equipos a un torneo de fútbol
...
public class PantallaAnadirEquipo extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.futbol_activity_pantalla_anadir_equipo);
		...
		llAnadir = (LinearLayout) findViewById(R.id.llFutbolGestionTorneoEquiposSeleccionados);
		llBuscar = (LinearLayout) findViewById(R.id.llFutbolGestionTorneoEquiposBuscar);
	...
	private void cargarEquipos(){
		...
		lista_equipos.clear();
		lista_equipos = PantallaInicial.bd.obtenerEquiposPorNombre(buscar, 1, getIntent().getIntExtra("id_torneo",-1));
		...
		while(it.hasNext()){
			...
			if(lista_equipos_seleccionados.indexOf(equipo.getId())<0){
				...
				LinearLayout ll = (LinearLayout) inflater.inflate(R.layout.futbol_fila_buscar_equipo, null);
	...
	private void cargarListaSeleccionada(View v){
		...
		LinearLayout ll = (LinearLayout) inflater.inflate(R.layout.futbol_fila_anadir_equipo, null);
		...
		img.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				//Log.d("TAG_Antes_de_Borrar",Integer.parseInt(v.getTag()+"")+"");
				lista_equipos_seleccionados.remove(Integer.parseInt(v.getTag()+""));
				llAnadir.removeViewAt(Integer.parseInt(v.getTag()+"")); 
				posicion_anadir--;
				//v.setTag(Integer.parseInt(v.getTag()+"")-1);
				cargarEquipos();
				//Log.d("TAG_Despues_de_Borrar",Integer.parseInt(v.getTag()+"")+"");
